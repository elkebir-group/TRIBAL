...
def main():
    ...
    parser_preprocess.add_argument( "--min-size", type=int, default=5,
    ...
    parser_preprocess.add_argument("--verbose", action="store_true",
    ...
    parser_preprocess.add_argument("--dnapars", type=str,
            help = "path to the dnapars executable if not on system path")
    

     # tribal command
    parser_tribal = subparsers.add_parser("tribal", help="B cell lineage tree inference")
    parser_tribal.set_defaults(func=tribal)
...
def preprocess(args):
    ...
    processor = Preprocessor(isotypes, min_size=args.min_size, use_light_chain=not args.heavy,
                             verbose=args.verbose)
    input_df = pd.read_csv(args.data)
    roots = pd.read_csv(args.roots)
    clonotypes, df = processor.preprocess(input_df, roots, cores=args.cores, dnapars_path=args.dnapars)
...
def save_results(outpath, best_trees, clonotypes, pngs=False, isotype_mapping=None):
    ...
    for clono, tree_list in best_trees.items():
        ...
        for i,lt in enumerate(tree_list):
            ...
            ut.write_fasta(f"{outpath}/{clono}_sequences_{i}.fasta", inferred_seqs)
            ut.save_dict(f"{outpath}/{clono}_isotypes_{i}.csv",lt.isotypes)
